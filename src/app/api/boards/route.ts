...
const getBoards = async (res: NextApiResponse, session) => {
  try {
...
const createBoard = async (
  ...
  if (boardData.columns) {
    const set = new Set();
    if (
      boardData.columns.some(
        (col) => set.size === (set.add(col.name), set.size)
      )
    ) {
  ...
  try {
    const newBoard = await db.board.create(payload);
